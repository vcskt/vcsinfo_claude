...
async function main() {
  ...
  // Create admin user (password from env, fallback for local dev only)
  const adminPassword = await argon2.hash(process.env.SEED_ADMIN_PASSWORD || 'changeme-admin');
  ...
  // Create client user (password from env, fallback for local dev only)
  const clientPassword = await argon2.hash(process.env.SEED_CLIENT_PASSWORD || 'changeme-client');
  ...
  const projects = await Promise.all([
    prisma.project.create({
      data: {
        ...
        content:
          'Projeto de implementação de um sistema ERP utilizando Protheus com customizações avançadas em ADVPL e integração com front-end em Angular.',
    ...
    prisma.project.create({
      data: {
        ...
        content:
          'Sistema de dashboards e relatórios interativos que extraem dados do Protheus em tempo real com visualizações em charts e gráficos.',
  ...
  const posts = await Promise.all([
    prisma.blogPost.create({
      data: {
        ...
        excerpt:
          'Saiba como criar customizações poderosas e eficientes em ADVPL para o Protheus, com boas práticas e exemplos práticos.',
        content:
          'Este artigo apresenta as melhores práticas para desenvolvimento de customizações em ADVPL...',
        category: 'Técnico',
    ...
    prisma.blogPost.create({
      data: {
        ...
        content:
          'Com PO-UI e Angular, é possível criar interfaces web modernas e integradas ao Protheus...',
        category: 'Frontend',
    ...
    prisma.blogPost.create({
      data: {
        ...
        content:
          'This article covers the best practices for integrating ADVPL with modern frontend technologies...',
        category: 'Technical',
  ...
  const testimonials = await Promise.all([
    prisma.testimonial.create({
      data: {
        ...
        quote:
          'A expertise da VCS Info em Protheus transformou completamente nossa infraestrutura. Recomendo fortemente!',
        published: true,
      },
    }),
    prisma.testimonial.create({
      data: {
        ...
        quote:
          'Projeto entregue no prazo, com qualidade excepcional. A equipe é muito profissional e dedicada.',
        published: true,
      },
    }),
  ]);
